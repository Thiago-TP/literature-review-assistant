...
def test_dashboard_aggregates_tags_ratings_and_scores(client, project):
    ...
    paper_a = client.post(f"/api/projects/{project['id']}/papers", json={"title": "A"}).json()["paper"]
    paper_b = client.post(f"/api/projects/{project['id']}/papers", json={"title": "B"}).json()["paper"]
    ...
    sufficient_entry = next(e for e in body["tag_distribution"] if e["option_id"] == sufficient["id"])
...
def test_dashboard_fully_tagged_count(client, project):
    ...
    fully_tagged = client.post(f"/api/projects/{project['id']}/papers", json={"title": "Full"}).json()["paper"]
    partially_tagged = client.post(f"/api/projects/{project['id']}/papers", json={"title": "Partial"}).json()["paper"]

    client.post(f"/api/projects/{project['id']}/papers/{fully_tagged['id']}/tags/{adherence['options'][0]['id']}")
    client.post(f"/api/projects/{project['id']}/papers/{fully_tagged['id']}/tags/{contribution['options'][0]['id']}")
    client.post(f"/api/projects/{project['id']}/papers/{partially_tagged['id']}/tags/{adherence['options'][0]['id']}")
