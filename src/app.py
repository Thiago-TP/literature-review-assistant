...
def _previous_progress_uploader_callback() -> None:
    """
    Load a previously saved review session from a JSON file.

    Handles the on_change event when a user uploads a JSON file containing
    a previous session's progress. The JSON file is expected to be in the format
    generated by the download button (list of dicts with paper tags and notes).

    When loaded:
    1. Reads the JSON file from the file uploader widget
    2. Deserializes it into session_progress structure
    3. Replaces any existing progress with the loaded data
    4. Displays a success message to the user

    The loaded progress is immediately available for review or modification.
    Users can continue tagging from where they left off or modify existing tags.
    """
...
def _update_progress_with_assigned_tags(field_name: str, key: str) -> None:
    """
    Persist tag assignments to session state when user modifies tags.

    Called as the on_change callback when user selects/deselects tags in the UI.

    Args:
        field_name: Name of the tag field being modified (e.g., "Adherence")
        key: Streamlit session state key containing the selected tags for this field

    Updates:
        session_progress[current_work_index][field_name] with the list of selected tags

    This ensures that all tag changes are immediately persisted to the session,
    enabling proper JSON export and progress visualization.
    """
